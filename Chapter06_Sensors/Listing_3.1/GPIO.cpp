...
GPIO::GPIO (int id, int mode) {
   ...
   cout << "Trying to set up the GPIO pin: " << id << endl;
   unexportGPIO();
   exportGPIO();
   setMode(mode);
   cout << "Setting the GPIO pin was a success!\n\n";
}
...
GPIO::~GPIO() {
   ...
   this->unexportGPIO();
   cout << "Destroying the GPIO_PIN with path: " << path << endl;
}
...
int GPIO::setMode(int mode) {
   ...
   case OUTPUT:
      if (writeFile(path, "direction", "out") != 0) 
         ...
         cout << "Set the pin direction as OUTPUT" << endl;
      break;
   case INPUT:
      if (writeFile(path, "direction", "in") != 0) 
         throw "Error to set the pin direction as INPUT";
      else
         cout << "Set the pin direction as INPUT" << endl;
      break;   
   }
   return 0;
}
